...
data <- data %>% 
  ...
  mutate(country = ifelse(country == "Hong Kong S.A.R. / China", "Hong Kong", country)) %>%
...
data.filter<-data %>% 
    filter(!is.na(morality_wis_analogy) | 
           ...
           !is.na(wisdom_culture)|
             !is.na(comments)) %>%
  select(CountryCode,Progress, Duration,position,position2,ResponseId,field,country,institution_type,prior_research:education,
...
Moral.open <- data %>% 
  ...
  mutate(Fixed=ifelse(FM_FINAL==1,1,0), Malleable=ifelse(FM_FINAL==2,1,0)) %>% 
    select(CountryCode,Progress, Duration,position2,ResponseId,field,prior_research:education,moral_wis_comments,morality_wis_analogy,wis_psych_char,
         FRMW_FINAL,  FRWM_FINAL, FRJF_FINAL,TRMW_FINAL,TRWM_FINAL,TRHOC_FINAL,PR_FINAL,DR_FINAL,PROCESS.MECH_FINAL,PROCESS.LIFE_FINAL,CONSTRUCT_FINAL,Fixed,Malleable) %>% 
    rename(Position = position2, Education = education, Field = field)
...
Moral.int <- data %>% 
    ...
    select(CountryCode,Progress, Duration,position2,ResponseId,field,prior_research:education,moral_wis_comments,morality_wis_analogy,wis_psych_char,
         FRMW_FINAL,  FRWM_FINAL, FRJF_FINAL,TRMW_FINAL,TRWM_FINAL,TRHOC_FINAL,PR_FINAL,DR_FINAL,PROCESS.MECH_FINAL,PROCESS.LIFE_FINAL,CONSTRUCT_FINAL,Intention_FINAL, Action_FINAL, MDW_FINAL) %>% 
    rename(Position = position2, Education = education, Field = field)
...
labels.INT<-c(
  ...
  MDW_FINAL = "Morality Depands on WIsdom")
