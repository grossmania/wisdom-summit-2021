...
```{r}
setwd("~/GitHub/wisdom-summit-2021/presummit survey")
...
total$Finished<-as.numeric(total$Finished)
total$Intention_FINAL<-as.numeric(total$Intention_FINAL)
total$Action_FINAL<-as.numeric(total$Action_FINAL)
total$MDW_FINAL<-as.numeric(total$MDW_FINAL)

lay<-read.csv('2021Wisdom Summit.Lay people.csv')
lay$sample<-"lay"
lay$Finished<-as.numeric(lay$Finished)
lay$Intention_FINAL<-as.numeric(lay$Intention_FINAL)
lay$Action_FINAL<-as.numeric(lay$Action_FINAL)
lay$MDW_FINAL<-as.numeric(lay$MDW_FINAL)
...
## filtering for lay people and combine
...
#how many completed more than 70?
table(lay$Progress>70) #407
...
```{r}
prop.table(table(lay.filter$sex))

table(lay.filter$gender) #only some filled it out, forgot to include at the beginning.

summary(lay.filter$age)
sd(lay.filter$age, na.rm=T)

table(lay.filter$ethnicity)
#31 white, 3, hispanic, 1 east Indian, 1 black, 5 asian
...
#how many responses?
length(WisMor.lay$M2W_Necessary)
WisMor.lay.long<-WisMor.lay %>% pivot_longer(M2W_Relevant:W2M_Sufficient,names_sep="_",names_to=c("Group","Category"),values_to="Proportion")

WisMor.lay.long$Group.F[WisMor.lay.long$Group=="M2W"]<-"Morality to Wisdom"
WisMor.lay.long$Group.F[WisMor.lay.long$Group=="W2M"]<-"Wisdom to Morality"


Wis.Mor.lay.Summary<-WisMor.lay.long %>%
  group_by(Group.F, Category) %>%
  summarise(Prop = mean(Proportion)) %>%
  mutate(Percent = Prop*100) %>% ungroup() %>%  arrange(Group.F, Percent) 

morals.lay <- Wis.Mor.lay.Summary %>%
  # 1. Remove grouping
  ungroup() %>%
  # 2. Arrange by
  #   i.  facet group
...
#lollipop chart
morals.lay %>% ggplot(aes(x=order, y=Percent, color=Group.F)) +  
geom_point(size=3) + 
  geom_segment(aes(x=order, 
                   xend=order, 
                   y=0, 
                   yend=Percent)) +facet_wrap(vars(Group.F), scales = "free")+
  scale_x_continuous(   # Add categories to axis
    breaks = morals.lay$order,
    labels = morals.lay$Category)+theme_minimal(base_size = 14) +geom_hline(yintercept =50, linetype='dashed', color='black', 14)+theme(legend.position="none")+
scale_color_aaas()+ labs(colour = "Group",x="",y="% of the Sample") 

xtabs(Proportion~Group.F+Category,WisMor.lay.long)
plot(xtabs(Proportion~Category+Group.F,WisMor.lay.long))

assocstats(xtabs(Proportion~Group.F+Category,WisMor.lay.long))

#GLMER ANALYSIS
glmer.morals.lay<-glmer(Proportion~Group.F*Category+ (1|ResponseId),data =WisMor.lay.long, family=binomial(link="logit") )
summ(glmer.morals.lay, digits=3)
Anova(glmer.morals.lay,type=3)

emmeans(glmer.morals.lay,pairwise ~Category|Group.F,  trans = "response")

morals.lay.estimates<-as.data.frame((emmeans(glmer.morals.lay,pairwise ~Category|Group.F,  trans = "response"))$emmeans)
pd <- position_dodge(0.7) # move them .07 to the left and right

morals.lay.estimates %>% 
 ggplot(aes(x = Category, y = prob, colour = Group.F, fill=Group.F))+
 geom_pointrange(aes(ymin=asymp.LCL, ymax=asymp.UCL), position=pd)+  theme_minimal() +
theme(legend.position="top") +scale_color_d3()+scale_fill_d3()+  
  labs(colour = "",fill="", x="",y="Proportion +/-95%CI") 

emmeans(glmer.morals.lay,pairwise ~Group.F|Category,  trans = "response")
...
#how many responses?
length(WisMor.both$M2W_Necessary)
WisMor.both.long<-WisMor.both %>% pivot_longer(M2W_Relevant:W2M_Sufficient,names_sep="_",names_to=c("Group","Category"),values_to="Proportion")

WisMor.both.long$Group.F[WisMor.both.long$Group=="M2W"]<-"Morality to Wisdom"
WisMor.both.long$Group.F[WisMor.both.long$Group=="W2M"]<-"Wisdom to Morality"


Wis.Mor.both.Summary<-WisMor.both.long %>%
  group_by(sample,Group.F, Category) %>%
  summarise(Prop = mean(Proportion)) %>%
  mutate(Percent = Prop*100) %>% ungroup() %>%  arrange(sample,Group.F, Percent) 

morals.both <- Wis.Mor.both.Summary %>%
  # 1. Remove grouping
  ungroup() %>%
  # 2. Arrange by
  #   i.  facet group
  #   ii. bar height
  arrange(sample,Group.F, Percent) %>%
  # 3. Add order column of row numbers
  mutate(order = row_number())

#lollipop chart
morals.both %>% ggplot(aes(x=order, y=Percent, color=sample)) +  
geom_point(size=3) + 
  geom_segment(aes(x=order, 
                   xend=order, 
                   y=0, 
                   yend=Percent)) +facet_wrap(vars(Group.F, sample), scales = "free")+
  scale_x_continuous(   # Add categories to axis
    breaks = morals.both$order,
    labels = morals.both$Category)+theme_minimal(base_size = 14) +geom_hline(yintercept =50, linetype='dashed', color='black', 14)+theme(legend.position="none")+
scale_color_aaas()+ labs(colour = "Group",x="",y="% of the Sample") 

xtabs(Proportion~sample+Group.F+Category,WisMor.both.long)
plot(xtabs(Proportion~Category+Group.F,WisMor.both.long))

assocstats(xtabs(Proportion~sample+Group.F+Category,WisMor.both.long))

#GLMER ANALYSIS
glmer.morals.both<-glmer(Proportion~sample*Group.F*Category+ (1|ResponseId),data =WisMor.both.long, family=binomial(link="logit") )
summ(glmer.morals.both, digits=3)
Anova(glmer.morals.both,type=3)

emmeans(glmer.morals.both,pairwise ~Category|Group.F|sample,  trans = "response")

morals.both.estimates<-as.data.frame((emmeans(glmer.morals.both,pairwise ~Category|Group.F|sample,  trans = "response"))$emmeans)
pd <- position_dodge(0.7) # move them .07 to the left and right

morals.both.estimates %>% 
 ggplot(aes(x = Category, y = prob, colour = sample))+facet_wrap(vars(Group.F))+
 geom_pointrange(aes(ymin=asymp.LCL, ymax=asymp.UCL), position=pd)+  theme_minimal() +
theme(legend.position="top") +scale_color_d3()+scale_fill_d3()+  
  labs(colour = "",fill="", x="",y="Proportion +/-95%CI") 

emmeans(glmer.morals.both,pairwise ~sample|Group.F|Category,  trans = "response")
...
### academics & lay people
```{r}

Moral.open.both <- WisMor.both %>% 
    filter(!is.na(FRMW_FINAL)) %>%
  mutate(Fixed=ifelse(FM_FINAL==1,1,0), Malleable=ifelse(FM_FINAL==2,1,0)) %>% 
    select(sample, CountryCode,Progress, Duration,Position,ResponseId,Field,prior_research:education,moral_wis_comments,morality_wis_analogy,wis_psych_char,
         FRMW_FINAL,  FRWM_FINAL, FRJF_FINAL,TRMW_FINAL,TRWM_FINAL,TRHOC_FINAL,PR_FINAL,DR_FINAL,CONSTRUCT_FINAL,Fixed,Malleable, Social,Personality,Moral_Psychology,Moral_philosophy,Education,CogSci, Developmental, M2W_Relevant:W2M_Unrelated) 
...
labels.N<-c(
  ...
  PR_FINAL ="7. Probabilistic",
  DR_FINAL = "8. Deterministic",
  CONSTRUCT_FINAL = "9. Theme:\nConstructivism")

#how many responses?
length(Moral.open.both$FRMW_FINAL)
Moral.open.both.long<-Moral.open.both %>% pivot_longer(FRMW_FINAL:Malleable,names_to="Category",values_to="Score")

Moral.open.both.Summary<-Moral.open.both.long %>%
  group_by(sample,Category) %>%
  summarise(Prop = mean(Score, na.rm=T)) %>%
  mutate(Percent = Prop*100) %>% ungroup() %>%  arrange(Percent) 


Moral.open.both.Summary %>% arrange(Percent) %>% 
  ggplot(aes(x=Category, y=Percent, fill=sample)) +
  geom_bar(position="dodge", stat="identity") +
theme_minimal()+  # Draw dashed lines
    coord_flip()+scale_fill_aaas()+scale_x_discrete(labels=labels, name="")


lmer.open.morals.both<-glmer(Score~sample*Category+ (1|ResponseId),data =Moral.open.both.long, family=binomial(link="logit") )
Anova(lmer.open.morals.both,type=3)

emmeans(lmer.open.morals.both,pairwise ~Category|sample,  trans = "response")

morals.open.both.estimates<-as.data.frame((emmeans(lmer.open.morals.both,pairwise ~Category|sample,  trans = "response"))$emmeans)
pd <- position_dodge(0.7) # move them .07 to the left and right

morals.open.both.estimates %>% 
 ggplot(aes(x = Category, y = prob, colour = Category))+
 geom_pointrange(aes(ymin=asymp.LCL, ymax=asymp.UCL), position=pd)+  theme_minimal() +facet_wrap(vars(sample),nrow =2)+coord_flip()+
theme(legend.position="none") +scale_color_d3(palette="category20")+scale_fill_d3(palette="category20")+  
  labs(colour = "",fill="", x="",y="Proportion +/-95%CI") +scale_x_discrete(labels=labels, name="")
...
## among both academics and lay people

```{r}
#compare those who indicated that morality is necessary for wisdom to those who did not select it.

Moral.open.both.Summary.Mness<-Moral.open.both.long %>%
  group_by(M2W_Necessary, Category,sample) %>%
  summarise(Prop = mean(Score, na.rm=T)) %>%
  mutate(Percent = Prop*100) %>% ungroup() %>%  arrange(Percent) 


Moral.open.both.Summary.Mness %>% arrange(Percent) %>% 
  ggplot(aes(x=Category, y=Percent, fill=as.factor(M2W_Necessary))) +
  geom_bar(position="dodge", stat="identity") +facet_wrap(vars(sample))+
theme_minimal()+  # Draw dashed lines
    coord_flip()+scale_fill_d3()+scale_x_discrete(labels=labels, name="")+
  labs(fill = "Morals Necessary\nfor Wisdom",x="",y="% of the Sample") +scale_x_discrete(labels=labels, name="")+coord_flip()


#compare those who indicated that wisdom is necessary for morality

Moral.open.both.Summary.Wness<-Moral.open.both.long %>%
  group_by(Category,W2M_Necessary, sample) %>%
  summarise(Prop = mean(Score, na.rm=T)) %>%
  mutate(Percent = Prop*100) %>% ungroup() %>%  arrange(Percent) 

Moral.open.both.Summary.Wness %>% arrange(Percent) %>% 
  ggplot(aes(x=Category, y=Percent, fill=as.factor(W2M_Necessary))) +
  geom_bar(position="dodge", stat="identity") +facet_wrap(vars(sample))+
theme_minimal()+  # Draw dashed lines
    coord_flip()+scale_fill_d3()+scale_x_discrete(labels=labels, name="")+
...
#total y group - looks off (hard to figure out)
univ2 %>%
